...
For the Shiny App, once a user input his/her interested job field and location, our app will display a weighted word graph. If the user is interested in details of job postings, he/she can choose to display a detailed table with top N frequent words if applicable, in which N is also a parameter given by user. The user can choose N to be an integer from 10 to 100.  The user is able to put any job field in the input text box. The app will take the text and perform web scraping task with input text. The location value accepts either city or state.  In general, Our analysis is highly customized. From the visualization output, a user could easily identify the most matched skillsets along with duties for his/her ideal job.      

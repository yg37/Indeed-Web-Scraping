...
#Beautiful Bananas Final Project
...
At the beginning, we used a keyword, i.e. “Statistical Analyst”, to get search result pages with 10 entries on each one. Then we went to each job posting page to download the html file. We need to find elements on a particular job posting. Because indeed.com does not provide a format template for employers, so it is very hard to extract the desired text chunks since they are discrete and not in order. We used a combination of keywords’ roots and regular expression to extract the chuck.    
        
        
```{r}
#Former combination of keywords’ roots and regular expression
...
The success ratio of posting scraping was around 50%, so we decided to deploy another approach. We abandoned job postings and turned back to search result pages. On the search result page, there is a description for each job entry. So we extracted descriptions and performed the text mining. Details are as followed.   
        
Our first step is cleaning the text of numbers, stop words, stem words and any html node tags resulting from web scraping. Then we perform tf-idf to the document term matrix to weigh the words based on their frequencies. After studying the scraped descriptions, we realize that qualifications appear mostly as nouns while verbs are mostly related to responsibilities. As a result, we use the openNLP package in R to find the part-of-speech of each word and retain only nouns for our analysis. At this point, we have filtered out the irrelevant and the most frequent, non-informative words. Then we use word cloud to visualize the highest weighted words.       

```{r}

source("get_indeed.R")
source("parse_indeed.R")
source("Scrape Descriptions.R")


```

Above is an example for wordcloud output.


###3.Visualization        
       
For the Shiny App, once a user choose his/her interested job field and location, our app will display a weighted word graph and a table with top 30 frequent words if applicable. The user can choose from a general job field, including Statistics, Computer Science, Management, Accounting, Marketing. Our analysis are based on each state. From the visualization output, a user could easily identify the most matched skillsets along with duties for his/her ideal job.      
...
The biggest challenge is to improve the accuracy of keyword match in the process of web text mining. Since employers write the job posting on their own, the contents of different postings vary so much, from organization to format. Sometimes they even do not have segments, putting everything in a single paragraph instead. The keywords we previously thought about are also difficult to find. For example, a posting list bullet points for “requirements” while another have sentences starting with “A qualified candidate should”.        
       
To solve the problem, we process job descriptions on the search result page, finding nouns to serve as qualifications. This involve manual work to read some job descriptions to look for potential patterns. There is no doubt we will miss something with this approach. For the long term process, it is necessary to figure out how to improve the text matching rate continuously.
